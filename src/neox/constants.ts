...
export const NEOX_T4_CHAIN_KEY = "neox-t4" as const;
export const NEOX_T4_CHAIN_ID = 12227332;
export const NEOX_T4_CHAIN_ID_HEX = "0xba9304" as Hex;
export const NEOX_MAINNET_CHAIN_ID = 47763;

export const NEOX_T4_IDENTITY_REGISTRY =
    "0x8004A856a396D08d31E597a867B1D8273901e641" as Address;
...
export function isNeoxChain(chain: string): boolean {
    return chain === NEOX_T4_CHAIN_KEY;
}

export function agentRegistryCaip(registry: string, chainId = NEOX_T4_CHAIN_ID): string {
...
export function assertTestnetWritesAllowed(chainId: number): void {
    if (chainId === NEOX_MAINNET_CHAIN_ID) {
        throw new Error(
            `Mainnet writes are not supported (chainId ${chainId}). Use Neo X T4 (${NEOX_T4_CHAIN_ID}).`
        );
    }
    if (chainId !== NEOX_T4_CHAIN_ID) {
        throw new Error(
            `Refusing write: eth_chainId is ${chainId}, expected Neo X T4 ${NEOX_T4_CHAIN_ID}.`
        );
    }
}
